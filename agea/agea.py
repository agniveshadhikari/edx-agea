"""
This modeule provides an XBlock which allows an instructor to
include a problem where a question/template and a solution file
will be uploaded and the students will be able to download
the template and upload their solutions, which will be
...
class ExcelSheetAssessmentXBlock(XBlock):
    """
    The XBlock class, contains all the fields and the methods
    """
    has_score = True
    icon_class = 'problem'
    ...
    question = String(
        ...
        help="This is the question text that is shown to the student "
             "alongside the uploaded question file"
    )
    ...
    max_attempts = Integer(
        ...
        help=("Defines the number of times a student attemot this problem;"
              "infinite if blank"),
        scope=Scope.settings
    )
    ...
    raw_answer = Dict(
        default={},
        help=("Dict internally used to store the sha1, mimetype, "
              "filename of the last uploaded assignment"),
        scope=Scope.user_state
    )
    ...
    raw_question = Dict(
        default={},
        help=("Dict internally used to store the sha1, mimetype, "
              "filename of the last uploaded question"),
        scope=Scope.settings
    )
    ...
    raw_solution = Dict(
        default={},
        help=("Dict internally used to store the sha1, mimetype, "
              "filename of the last uploaded solution"),
        scope=Scope.settings
    )

    def max_score(self):
        """
        Reuturns the maximum attainable score, stored in points in our case
        """
        return self.points
    ...
    def block_id(self):
        """
        Returns a distinct id of the XBlock instance
        """
    ...
    def student_state(self):
        """
        Returns the context for rendering the student view
        in the form of a dictionary
        """
        submission = self.get_submission()
        if submission:
            uploaded_submission = submission.get(
                "answer").get("filename", None)
    ...
    def studio_state(self):
        """
        Returns the context for rendering the studio view
        in the form of a dictionary
        """
        ...
        if submission:
            uploaded_submission = submission.get("solution").get(
                "filename", None)
            if uploaded_submission:
    ...
    def studio_view(self, context=None):
        ...
        def none_to_empty(data):
            """
            Converts none to empty string and returns
            """
        ...
        if submission:
            ...
            if uploaded_submission:
                ...
                if actual < points:
                    raise JsonHandlerError(
                        400,
                        '"Score to be graded out of" should be "\
                        "less than equal to the maximum attainable score "\
                        "for the question paper you uploaded')
        self.save()
    ...
    def upload_assignment(self, request, suffix=''):
        #pylint: disable=unused-argument
        """
        Uploads the student file on local disk, then calls storage api,
        grades the file, and then deletes the file from local disk
        """
        ...
        try:
            storage.store_data(
                ...
                str(self.location.block_id), file(IMAGEDIFF_ROOT + path))
    ...
    def upload_question(self, request, suffix=''):
        ...
        try:
            storage.store_data(str(self.course_id),
                "question", str(self.location.block_id),
                file(IMAGEDIFF_ROOT + path))
        except PersonValueError:
            LOG.info("storage api upload failed:")
            LOG.info("peson argument cant be an empty string")
        except DepartmentValueError:
    ...
    def download_assignment(self, request, suffix=''):
        # pylint: disable=unused-argument
        """
        Downloads the student's answer file from storage api
    ...
    def download_question(self, request, suffix=''):
        ...
        try:
            ...
            return Response(
                ...
                content_disposition="attachment; filename=" +
                    self.raw_question["filename"].encode('utf-8'))
    ...
    def grade_this_guy(self):
        """
        Generates the path for the three files to be compared,
        passes them to the grader function, popuates the
        score field, runtime.publish-es the grades
        so that it shows up in the progress page
        """
        answer = self._file_storage_path(
            self.raw_answer['sha1'],
            self.raw_answer['filename'])
        question = self._question_storage_path(
            self.raw_question['sha1'],
            self.raw_question['filename'])
        solution = self._solution_storage_path(
            self.raw_solution['sha1'],
            self.raw_solution['filename'])
...
def workbench_scenario():
    """
    You don't know me, but trust me, this is really really important.
    """
    return[
